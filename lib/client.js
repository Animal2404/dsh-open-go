...
function QuotaWidget(props) {
  ...
  return React.createElement('div', {
      ...
      React.createElement('div', { key: 'acts', style: { display: 'flex', alignItems: 'center', gap: '2px', flex: 'none' } }, [
        React.createElement('button', {
          ...
          style: {
            ...
            color: mutedColor, fontSize: '13px', lineHeight: 1, padding: '2px 4px', borderRadius: '6px',
          },
        }, '⚙'),
        React.createElement('button', {
          key: 'r',
    ...
    React.createElement('div', {
      ...
      React.createElement('span', { key: 't', style: { fontSize: '12px', fontWeight: 600, color: mainColor } }, '额度'),
    ]),
    React.createElement('div', {
      key: 'quota-card',
      style: {
              ...
              official.todayModels && official.todayModels.length > 0
                ? official.todayModels.map(function (m, i) {
                    ...
                    return React.createElement('div', {
                      ...
                      React.createElement('span', { key: 'l', style: { flex: '0 1 auto', maxWidth: '46%', fontSize: '10px', color: mutedColor, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', minWidth: '0' }, title: m.model }, shortModel(m.model)),
                      React.createElement('div', { key: 'bar', style: barBase }, [
                        React.createElement('div', { key: 'fill', style: Object.assign({}, fillBase, { width: pct + '%', background: 'light-dark(' + color + ',' + colorDark + ')' }) }),
                      ]),
                      React.createElement('span', { key: 'p', style: { flex: 'none', width: '30px', textAlign: 'right', fontSize: '10px', color: mutedColor, fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' } }, pct + '%'),
                      React.createElement('span', { key: 'v', style: { flex: 'none', fontSize: '10px', fontWeight: 600, color: mainColor, fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' } }, fmtUsd(m.costUsd)),
                    ])
                  })
    ...
    cfgOpen ? React.createElement('div', {
      ...
      React.createElement('div', {
        ...
        React.createElement('label', { key: 'l1', style: { display: 'flex', flexDirection: 'column', gap: '4px', fontSize: '11px', color: mutedColor } }, [
          ...
          React.createElement('input', {
            ...
            style: {
              padding: '7px 9px', borderRadius: '8px', fontSize: '12px',
              border: '1px solid var(--dsw-alias-border-l2, rgba(127,127,127,.3))',
              background: 'var(--dsw-alias-bg-layer-2, #26262a)',
              color: 'inherit', outline: 'none',
            },
          }),
          React.createElement('div', { key: 'wh', style: { fontSize: '10px', color: mutedColor, lineHeight: 1.5 } }, '它标识你在 opencode.ai 的哪个工作区：官方账单接口靠它定位查哪份用量（一个账号下可能有多个工作区，账单分开统计）。打开 https://opencode.ai/workspace/ 用量页，地址栏里 wrk_... 就是它。额度功能用 API key，不依赖此项。'),
        ]),
        React.createElement('label', { key: 'l2', style: { display: 'flex', flexDirection: 'column', gap: '4px', fontSize: '11px', color: mutedColor } }, [
          '控制台 Cookie（auth=... 完整值）',
          React.createElement('input', {
            type: 'password',
        ...
        cfgData.err ? React.createElement('div', { key: 'e', style: { fontSize: '11px', color: 'var(--dsw-alias-state-error-primary, #f87171)' } }, cfgData.err) : null,
        ...
        React.createElement('div', { key: 'hint', style: { fontSize: '10px', color: mutedColor, lineHeight: 1.5 } }, '获取 cookie：登录 opencode.ai 用量页 → F12 → Network（网络）→ 刷新页面 → 请求标头 cookie: 行复制 auth=... 值（或 Application → Cookies）。'),
      ]),
    ]) : null,
  ])
}
