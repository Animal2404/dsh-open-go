...
/**
 * dsh-opencode-quota（浏览器端）v0.7.2
 *
...
// 美元金额（官方账单消耗余额，显示 4 位小数；额度区不显示金额，不受影响）
function fmtUsd(n) {
  ...
  return React.createElement('div', {
    ...
    TIERS.map(function (t) {
      ...
      const row = React.createElement('div', {
        ...
        }, ok ? ('距重置 ' + (u && u.resetsIn ? u.resetsIn : '未知')) : ''),
      ])
          return row
    }),
    ]),
    ...
    cfgOpen ? React.createElement('div', {
      ...
      React.createElement('div', {
        ...
        React.createElement('label', { key: 'l1', style: { display: 'flex', flexDirection: 'column', gap: '4px', fontSize: '12px', color: '#c8d2e0' } }, [
          ...
          React.createElement('div', { key: 'wh', style: { fontSize: '11px', color: '#9aa8bd', lineHeight: 1.5 } }, '它标识你在 opencode.ai 的哪个工作区：官方账单接口靠它定位查哪份用量（一个账号下可能有多个工作区，账单分开统计）。打开 https://opencode.ai/workspace/ 用量页，地址栏里 wrk_... 就是它。额度功能用 API key，不依赖此项。'),
        ]),
        React.createElement('label', { key: 'l2', style: { display: 'flex', flexDirection: 'column', gap: '4px', fontSize: '12px', color: '#c8d2e0' } }, [
          '控制台 Cookie（auth=... 完整值）',
          React.createElement('input', {
            type: 'password',
        ...
        cfgData.err ? React.createElement('div', { key: 'e', style: { fontSize: '12px', color: '#f87171' } }, cfgData.err) : null,
        cfgData.msg ? React.createElement('div', { key: 'msg', style: { fontSize: '12px', color: '#34d399' } }, cfgData.msg) : null,
        ...
        React.createElement('div', { key: 'hint', style: { fontSize: '11px', color: '#9aa8bd', lineHeight: 1.5 } }, '获取 cookie：登录 opencode.ai 用量页 → F12 → Network（网络）→ 刷新页面 → 请求标头 cookie: 行复制 auth=... 值（或 Application → Cookies）。'),
      ]),
    ]) : null,
  ])
}
