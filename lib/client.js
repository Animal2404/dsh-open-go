...
/**
 * dsh-opencode-quota（浏览器端）v0.9.8
 *
...
function TierRow(props) {
  ...
  // 展开时从 0 平滑长到**当前数据的真实进度**：终点就是 props.percent（/api/status 的实时值，不写死）；
  // 面板收起会卸载整块、重新展开即重新挂载 → 动画自然重播；数据变化时终点随之变化（补间从当前值出发）。
  // prefers-reduced-motion 下 useTweenNum 直接跳到终点，不播动画。
  const shown = useTweenNum(pct === null ? 0 : pct, 420, 0)
...
function ModelRow(props) {
  ...
  // 同三档条：展开时从 0 长到真实占比（终点 = props.percent）
  const shown = useTweenNum(pct, 420, 0)
