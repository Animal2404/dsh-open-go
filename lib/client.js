...
/**
 * dsh-opencode-quota（浏览器端）v0.9.0
 *
...
const CSS = [
  ...
  '.dshoq-scope{--oq-main:var(--dsw-alias-label-primary,#e6edf7);--oq-muted:var(--dsw-alias-label-secondary,#8fa3bf);--oq-track:var(--dsw-alias-border-l2,rgba(127,127,127,.22));--oq-border:var(--dsw-alias-border-l3,rgba(127,127,127,.25));--oq-line:light-dark(rgba(0,0,0,.1),rgba(127,127,127,.35));--oq-surface:var(--dsw-alias-fill-secondary,rgba(127,127,127,.06));--oq-card:light-dark(rgba(127,127,127,.055),rgba(160,160,170,.08));--oq-hover:rgba(127,127,127,.14);color:var(--oq-main);font:12px/1.5 -apple-system,BlinkMacSystemFont,"Segoe UI","Microsoft YaHei",sans-serif;font-variant-numeric:tabular-nums}',
  '.dshoq-wrap{flex:0 0 100%;width:100%}',

  // ── 收起态 pill ──
  '.dshoq-pill{display:flex;align-items:center;gap:7px;width:100%;box-sizing:border-box;padding:5px 8px;border-radius:9px;border:1px solid var(--oq-border);background:var(--oq-surface);color:var(--oq-main);font:inherit;text-align:left;cursor:pointer;white-space:nowrap;overflow:hidden;transition:background .18s ease,border-color .18s ease,transform .12s ease}',
  '.dshoq-pill:hover{background:var(--oq-hover)}',
  '.dshoq-pill:active{transform:scale(.985)}',
  '.dshoq-pill:focus-visible{outline:2px solid #4d6bfe;outline-offset:2px}',
  '.dshoq-badge{flex:none;font-size:10px;font-weight:800;letter-spacing:.6px;line-height:1;padding:3px 5px;border-radius:5px;border:1px solid var(--oq-line);background:light-dark(rgba(127,127,127,.1),rgba(255,255,255,.06));color:var(--oq-main)}',
  ...
  '.dshoq-chev--open{transform:rotate(180deg)}',

  // ── 展开面板 ──
  '.dshoq-panel{position:fixed;z-index:9998;box-sizing:border-box;display:flex;flex-direction:column;gap:8px;padding:10px 11px;border-radius:12px;background:light-dark(#fff,#232327);border:1px solid var(--oq-line);box-shadow:0 12px 38px rgba(0,0,0,.38);max-height:70vh;overflow-y:auto;scrollbar-width:thin;transform-origin:var(--oq-origin,50% 100%)}',
  ...
  '.dshoq-card{display:flex;flex-direction:column;gap:8px;border-radius:9px;padding:8px;background:var(--oq-card);border:1px solid var(--oq-line)}',
  ...
  // 窄 rail 态
  '.dshoq-rail{display:flex;flex-direction:column;align-items:center;justify-content:center;gap:3px;width:42px;height:48px;margin:4px auto;padding:0;border-radius:10px;border:1px solid var(--oq-border);background:var(--oq-surface);color:var(--oq-main);cursor:pointer;flex:0 0 100%;transition:background .18s ease,transform .12s ease}',
  '.dshoq-rail:hover{background:var(--oq-hover)}',
  ...
  '.dshoq-dialog{width:340px;max-width:calc(100vw - 40px);box-sizing:border-box;border-radius:12px;padding:14px;background:#1e1e20;border:1px solid rgba(127,127,127,.35);box-shadow:0 14px 44px rgba(0,0,0,.5);display:flex;flex-direction:column;gap:10px;color:#e6edf7;font:12px/1.5 -apple-system,BlinkMacSystemFont,"Segoe UI","Microsoft YaHei",sans-serif;font-variant-numeric:tabular-nums}',
  '.dshoq-dialog--in{animation:dshoq-pop .18s cubic-bezier(.16,1,.3,1) both}',
  '.dshoq-dlg-title{font-size:14px;font-weight:700}',
  '.dshoq-switch-row{display:flex;align-items:center;gap:10px;padding:8px 9px;border-radius:9px;background:rgba(127,127,127,.1);border:1px solid rgba(127,127,127,.22)}',
  ...
  '.dshoq-switch{position:relative;flex:none;margin-left:auto;width:38px;height:22px;padding:0;border-radius:11px;border:1px solid rgba(127,127,127,.35);background:rgba(127,127,127,.28);cursor:pointer;transition:background .22s ease,border-color .22s ease}',
  '.dshoq-switch[aria-checked="true"]{background:#4d6bfe;border-color:#4d6bfe}',
  ...
  '.dshoq-input{padding:7px 9px;border-radius:8px;font-size:13px;border:1px solid rgba(127,127,127,.35);background:#26262a;color:#e6edf7;outline:none;transition:border-color .16s ease,box-shadow .16s ease}',
  '.dshoq-input:focus{border-color:#4d6bfe;box-shadow:0 0 0 3px rgba(77,107,254,.22)}',
  ...
  '.dshoq-btn{padding:6px 13px;border-radius:8px;font-size:13px;cursor:pointer;border:1px solid rgba(127,127,127,.35);background:transparent;color:#c8d2e0;transition:background .16s ease,transform .12s ease}',
  '.dshoq-btn:hover{background:rgba(127,127,127,.16)}',
  '.dshoq-btn:active{transform:scale(.97)}',
  '.dshoq-btn:focus-visible{outline:2px solid #4d6bfe;outline-offset:1px}',
  '.dshoq-btn--primary{font-weight:600;background:rgba(127,127,127,.22);color:#e6edf7}',
...
function TierRow(props) {
  ...
  const shown = useTweenNum(pct === null ? 0 : pct, 520, 0)
...
function ModelRow(props) {
  ...
  const shown = useTweenNum(pct, 480, 0)
...
function MiniFill(props) {
  const shown = useTweenNum(typeof props.pct === 'number' ? props.pct : 0, 520, 0)
...
/** 箭头（收起 ▾ / 展开 ▴：同一个形状转过去，而不是换图标）。 */
function Chevron(props) {
  const s = props.size || 11
  return React.createElement('svg', {
    className: 'dshoq-chev' + (props.open ? ' dshoq-chev--open' : ''),
...
function QuotaWidget(props) {
  ...
  const setOpenPersist = React.useCallback(function (v) {
    const next = !!v
    setOpen(next)
    writeFlag(PANEL_OPEN_KEY, next)
  }, [])
  ...
  const placePanel = React.useCallback(function () {
    try {
      const pill = pillRef.current, panel = panelRef.current
  ...
  React.useEffect(function () {
    ...
    placePanel()
    const onResize = function () { placePanel() }
  ...
  }, [open, placePanel, setOpenPersist])
  // 数据 / 宽度 / 账单开关变化 → 重新锚定（高度会变）
  React.useEffect(function () {
    if (open) placePanel()
  }, [open, state, official, panelW, billing, placePanel])
  ...
  const toggleOpen = React.useCallback(function (e) {
    ...
    setOpen(function (v) {
      const next = !v
      writeFlag(PANEL_OPEN_KEY, next)
      if (next) refreshAll() // 展开时顺手刷一次（额度 + 已开启的账单）
      return next
    })
  }, [refreshAll])
  ...
  const panel = React.createElement('div', {
    ...
    style: {
      ...
      top: panelPos ? panelPos.top + 'px' : '0',
    ...
    // —— 额度 ——
    React.createElement('div', { key: 'qhead', className: 'dshoq-sect' }, [
      React.createElement('span', { key: 't', className: 'dshoq-sect-t' }, '额度'),
      warn ? React.createElement('span', { key: 'n', className: 'dshoq-sect-n' }, '仍显示上次数据') : null,
    ]),
