...
function QuotaWidget(props) {
  ...
  const refresh = React.useCallback(function () {
    ...
    window.fetch(HOST_API, { headers: { 'x-dsh-opencode-quota': '1' } })
  ...
  const refreshOfficial = React.useCallback(function () {
    let alive = true
    window.fetch(HOST_API_OFFICIAL, { headers: { 'x-dsh-opencode-quota': '1' } })
      .then(function (r) { return r.json() })
  ...
  if (!wide) {
    ...
    return React.createElement('button', {
      type: 'button',
      title: state.kind === 'err' ? ('Open GO 额度获取失败：' + state.error + '，点击重试') : 'Open GO 套餐额度（点击刷新）',
      onClick: refresh,
      style: {
