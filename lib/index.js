...
/** 从凭证文件或环境变量读取 opencode 控制台 auth cookie（登录 opencode.ai 后从
 * Cookie-Editor 等工具复制 auth=Fe26.2**... 的完整值）。用于抓官方用量页。 */
...
/** 从凭证文件或环境变量读取 opencode workspace id（官方用量页 URL 里的
 * wrk_...）。用于官方成本图表查询。 */
 ...
 * 返回 [{ date, model, costUsd }]，失败返回 null。 */
async function fetchOfficialCosts(wsId, year, month, tzOffset) {
  ...
  const timer = setTimeout(() => controller.abort(), 20000)
  try {
    const r = await fetch('https://opencode.ai/_server?id=15702f3a12ff8bff357f8c2aa154a17e65b746d5f6b96adc9002c86ee0c15205', {
      method: 'POST',
      headers: {
...
/** 调用 opencode usage 端点；失败返回 null。 */
    ...
    handler: async (req, res) => {
      ...
      try {
        if (url.pathname === API + '/status') {
          ...
          if (!usage) return sendJson(res, 200, { ok: false, error: 'opencode usage 端点请求失败（网络/鉴权/限流）' })
        ...
        if (url.pathname === API + '/official') {
          ...
          if (officialCache.payload && officialCache.key === cacheKey && now.getTime() - officialCache.at < OFFICIAL_CACHE_MS) {
            return sendJson(res, 200, { ...officialCache.payload, cached: true })
          }
          if (!wsId) return sendJson(res, 200, { ok: false, error: '缺少 workspace 参数' })
          // 官方成本图表 RPC：本月按日×模型（时区 +08:00，页面一致）
          const rows = await fetchOfficialCosts(wsId, now.getUTCFullYear(), now.getUTCMonth(), '+08:00')
          if (!rows) return sendJson(res, 200, { ok: false, error: '官方成本获取失败（未登录/网络/解析）' })
