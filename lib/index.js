...
async function cdpOpenUsage() {
  let list = []
  try { list = await cdpJson(CDP_BASE + '/json') } catch (e) { throw new Error('调试端口不可用') }
  let page = Array.isArray(list) ? list.find((t) => t.type === 'page') : null
  if (!page) {
    const created = await cdpJson(CDP_BASE + '/json/new?' + encodeURIComponent('about:blank'))
    page = created || null
  }
  const wsUrl = page && page.webSocketDebuggerUrl
  if (!wsUrl) throw new Error('无法获取调试页面地址')
  await cdpCall(wsUrl, 'Page.navigate', { url: 'https://opencode.ai/workspace/' }, 6000)
  return wsUrl
}

/** 从启用了调试端口的 Chrome 读取 opencode.ai 的 auth cookie。 */
async function grabCookieFromChrome() {
  let launched = null
  // 1) 检查调试端口；没有就先启动一个带调试端口的 Chrome（真实配置）
  let ready = false
  try { ready = Array.isArray(await cdpJson(CDP_BASE + '/json')) } catch (e) { /* not ready */ }
  if (!ready) {
    const exe = chromeExe()
    if (!exe) return { ok: false, error: '未找到 Chrome（非 Windows 或路径异常）' }
    const profile = join(process.env.LOCALAPPDATA || '', 'Google', 'Chrome', 'User Data')
    try {
      launched = spawn(exe, [
    ...
    } catch (e) { return { ok: false, error: '启动 Chrome 失败：' + ((e && e.message) || e) } }
    for (let i = 0; i < 25; i++) {
      await new Promise((r) => setTimeout(r, 1000))
      try { if (Array.isArray(await cdpJson(CDP_BASE + '/json'))) { ready = true; break } } catch (e) { /* retry */ }
    }
    if (!ready) {
      if (launched) { killTree(launched.pid) }
      return { ok: false, error: '无法连接到调试端口。Chrome 可能已在运行：请先完全关闭 Chrome（托盘退出）再点"自动抓取 Cookie"。' }
    }
  }
  // 2) 打开用量页并读取 cookie
  let wsUrl
  try { wsUrl = await cdpOpenUsage() } catch (e) { return { ok: false, error: '打开用量页失败：' + ((e && e.message) || e) } }
  ...
  if (hit && hit.value) {
    // 抓取成功：保存（保留现有 workspace id），并关闭我们启动的 Chrome
    if (launched) { killTree(launched.pid) }
    const auth = 'auth=' + hit.value
    const wsid = readWorkspaceId() || ''
    const saved = saveCredentials(wsid || undefined, auth)
    return { ok: true, saved, hasCookie: true, cookieMasked: maskSecret(auth), message: '已抓取并保存 auth cookie（立即生效）' }
  }
  // 未登录：保留 Chrome 供用户登录，再重试
  return { ok: false, error: '未检测到 opencode.ai 的 auth cookie（未登录？）。请在刚打开的 Chrome 里登录 opencode.ai（Continue with GitHub）后，再点一次"自动抓取 Cookie"。' }
}
