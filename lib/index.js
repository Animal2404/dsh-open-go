...
/** DSH 根目录（读写统一走 $DSH_HOME，非默认路径时保持一致）。 */
function dshHome() {
...
/** 面板里「显示官方账单」开关的说明文案（齿轮弹窗与设置面板**共用同一句**，避免两处措辞漂移）。 */
export const BILLING_DESC = '在面板里显示今日 / 本月消耗（需控制台 cookie）'

/** 设置面板 schema：workspace id + 控制台 cookie（secret 类型，UI 显示为密码框）+ 账单开关。 */
export const Config = z.object({
  ...
  billing: z.boolean().default(false).description(BILLING_DESC),
})

/** 设置值（可能为空对象）。 */
let settingsValue = {}

/** 已注册的设置 scope（保存路径要用它把值写回设置存储，使齿轮与设置面板同源）。 */
let settingsScope = null
/** 设置服务是否可写。注意 `writable` 在**服务**（`ctx.settings`）上，`SettingsScope` 只有 get/watch/update/replace。 */
let settingsWritable = false

/**
 * 把齿轮弹窗保存的值写回设置存储。
 * 齿轮与设置面板是同一份配置的两个入口，只写凭证文件会让设置面板一直显示旧值。
 * @param patch - 要合并进用户层的字段（workspaceId / consoleCookie / billing）
 * @returns 是否写入成功，以及不可写时的原因
 */
async function persistToSettings(patch) {
  if (!settingsScope) return { ok: false, reason: 'settings-unavailable' }
  if (!settingsWritable) return { ok: false, reason: 'settings-readonly' }
  try {
    await settingsScope.update(patch)
    return { ok: true }
  } catch (e) {
    return { ok: false, reason: (e && e.message) || 'settings-write-failed' }
  }
}
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    try {
      ...
      if (r.status !== 200) {
        ...
        // 只重试限流/服务端异常；4xx 是 cookie/参数问题，重试没有意义
        const retryable = r.status === 429 || r.status >= 500
        if (attempt < 3 && retryable) { await sleep(1500); continue }
        return null
      }
...
async function fetchUsage(key) {
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    try {
      ...
      if (r.status !== 200) {
        lastErr = 'HTTP ' + r.status
        // 只重试限流/服务端异常；401/403/404 等 4xx 立即返回明确错误
        const retryable = r.status === 429 || r.status >= 500
        if (attempt < 3 && retryable) { await sleep(1500 * attempt); continue }
...
export function apply(ctx) {
  ...
  try {
    ...
    settingsWritable = ctx.settings && ctx.settings.writable === true
    ...
    handler: async (req, res) => {
      ...
      try {
        if (url.pathname === API + '/config') {
          ...
          if (req.method === 'GET' || req.method === undefined) {
            ...
            return sendJson(res, 200, {
              ...
              billing: settingsValue.billing === true,
              settingsWritable: settingsWritable && !!settingsScope,
              // 值的来源口径：写回设置存储后，settingsValue 即存储里的值，两者不会再分叉
              source: settingsValue.workspaceId || settingsValue.consoleCookie ? 'settings' : 'credentials',
            })
          }
          // POST：保存 workspace id / cookie / 账单开关（凭证文件兜底 + 写回设置存储）
          if (req.method === 'POST') {
            ...
            const hasBilling = !!(j && typeof j.billing === 'boolean')
            // 校验口径：至少要有一项能落地的改动（空字符串=不修改，与弹窗「留空则不修改」一致）
            if (wsId === '' && cookie === '' && !hasBilling) {
              return sendJson(res, 200, { ok: false, error: '请至少填写一项（workspaceId / consoleCookie），或切换账单开关' })
            }
            const persisted = []
            if (hasBilling) {
              const r = await persistToSettings({ billing: j.billing })
              persisted.push(r.ok ? 'settings:billing' : 'billing:' + r.reason)
            }
            if (wsId || cookie) {
              ...
              // 同步写回设置存储：设置面板与齿轮从此共用同一份值
              const patch = {}
              if (wsId) patch.workspaceId = wsId
              if (cookie) patch.consoleCookie = cookie
              const r = await persistToSettings(patch)
              persisted.push(r.ok ? 'settings' : 'settings:' + r.reason)
            }
            return sendJson(res, 200, {
              ok: true,
              message: '已保存（立即生效）',
              persisted,
              billing: hasBilling ? j.billing : settingsValue.billing === true,
              settingsWritable: settingsWritable && !!settingsScope,
            })
          }
        ...
        if (url.pathname === API + '/official') {
          ...
          // 缓存键同样用北京时间月份，避免 +08:00 跨月瞬间（UTC 仍是上月）命中上月的缓存。
          const bjy = new Date(now.getTime() + 8 * 3600 * 1000)
          const cacheKey = wsId + '|' + bjy.getUTCFullYear() + '-' + bjy.getUTCMonth()
